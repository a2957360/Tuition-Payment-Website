<?php
    ...
    $stmt = $pdo->prepare("SELECT * FROM `house_table` WHERE `Id` = '$paymentId';");
    $stmt->execute();
    if($stmt != null){
    ...
    $stmt = $pdo->prepare("UPDATE `house_table` SET `OrderNo`='$orderid' WHERE `Id`='$paymentId';");
