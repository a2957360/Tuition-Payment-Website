<?php
    ...
    if($_POST['modifyvalue'] == '1'){
        ...
        $stmt = $pdo->prepare("UPDATE `other_table` 
                                JOIN (select * FROM `paymercant_table` WHERE `Id` = '$infoid') `merchant` 
                                SET `other_table`.`USDorCAD` = `merchant`.`USDorCAD`,
                                `other_table`.`CompanyName` = `merchant`.`CompanyName`, 
                                `other_table`.`AccountNo` = `merchant`.`AccountNo`, 
                                `other_table`.`Memo` = `merchant`.`Memo` WHERE `other_table`.`Id` = '$paymentId'");
        $stmt->execute();
          if($stmt != null){
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['modify'])  && $_POST['modifyvalue'] != '1') {
    ...
    $stmt = $pdo->prepare("INSERT INTO `other_table`(`Method`,`USDorCAD`, `CompanyName`, `CompanyAbb`, `CompanyAddress`, `AccountNo`, `Memo`,`ChargeFee`, `UserId`, `Status`, `Date`) SELECT '$method',`USDorCAD`,`CompanyName`, `CompanyAbb`, `CompanyAddress`, `AccountNo`, `Memo`, '$Percentage','$Id','0','$todaydate' FROM `paymercant_table` WHERE `Id` = '$infoid';");
    $stmt->execute();
      if($stmt != null){
        ...
        if($stmt != null){
          while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
            ...
            echo "<script> location.href='otherpayment.php'; </script>";
          }
        }
      }else{
        die('Error: ' . mysql_error());
...
<!--          <img class="img-fluid bannerlogo" alt="Responsive image" src="static/img/Logo.png">-->
      ...
      <div class="row">
        ...
        <div class="col-12 col-lg-5 m-auto text-center">
        <div class="row">
        <div class="col-2 col-lg-1 text-center ud20 paddingLeft20">
            ...
            <div class="col-lg-5 col-10 text-right ud20 pr10">
                ...
                <p class="float-right">加币CAD</p>&nbsp;
                  <div class="checkbox float-right">
                    <input class="" type="checkbox" id="currencycheck"/>
                    <label></label>
                  </div>
...
-->
        ...
        <?php
                $stmt = $pdo->prepare("SELECT * FROM `paymercant_table` WHERE `UserId` = '$Id';");
                $stmt->execute();
                if($stmt != null){
                  while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
...
-->
                        <div class="col-12 col-lg-12">
                            <h4 class="nomargin" style="display:inline-block"><?=$value['SchoolName']?><?=$value['CompanyName']?></h4>
                        </div> 
                    </div>
                </div>
                ...
                <input class="btn btn-primary btn-lg btncolor float-left btnpadding" type="submit" name="submit" value="选择" >
                </div>  
              </form>
