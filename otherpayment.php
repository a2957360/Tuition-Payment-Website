<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['modify'])) {
    ...
    if(isset($Percantage)){
        $stmt = $pdo->prepare("SELECT * FROM `chargefee_table` WHERE `Name` Like (SELECT `Method` FROM `other_table` WHERE  `Id`='$paymentId');");
        $stmt->execute();
        if($stmt != null){
          while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
        ...
        $stmt = $pdo->prepare("UPDATE `other_table` SET `Price`='$price',`ExceptDate`='$exceptdate',
        ...
        $stmt->execute();
          if($stmt != null){
              ...
              echo "<script> location.href='otherperson.php'; </script>";
        ...
        $stmt = $pdo->prepare("UPDATE `other_table` SET  `Price`='$price',`ExceptDate`='$exceptdate',`OrderNo`='$orderid' WHERE `Id`='$paymentId';");
        $stmt->execute();
          if($stmt != null){
            if($_POST['modifyvalue'] == '1'){
                    echo "<script> location.href='otherconfirm.php'; </script>";
