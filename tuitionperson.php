<?php
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['modify'])) {
    ...
    $stmt = $pdo->prepare("UPDATE `student_table` SET  `FirstName`='$firstname',`LastName`='$lastname',`Phone`='$phone',`Email`='$email' WHERE `Id`='$paymentId';");
    $stmt->execute();
      if($stmt != null){
            ...
            <div class="col-12 col-lg-12 m-auto">
              <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" enctype='multipart/form-data' class="row text-left">
                ...
                <div class="col-12 ">
                  ...
                  <input class="wide-80" id="firstname" value="<?=$FirstName?>" pattern="[a-zA-Z]{0,20}" type="text" name="firstname" required>
                </div>
                ...
                <div class="col-12 ">
                  ...
                  <input  class="wide-80" id="email" value="<?=$Email?>" type="email" name="email" required>
                    <p class="smFont grey">电子收据会发送至此邮箱</p>
                </div>    
    ...
    <script type="text/javascript">
      function defaultinfo(){
        if($("#customCheck1").prop('checked')){
          ...
          $("#phone").val("<?=$Phone?>");
          $("#email").val("<?=$Email?>");
          $("#firstname").attr('readonly','disabled');
          $("#lastname").attr('readonly','disabled');
          $("#phone").attr('readonly','disabled');
          $("#email").attr('readonly','disabled');
          $("#firstname").css('background-color','lightgrey');
          $("#lastname").css('background-color','lightgrey');
          $("#phone").css('background-color','lightgrey');
